@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between font-iran-sans">
            <div>
                <p class="text-sm text-gray-700 leading-5 dark:text-gray-400">
                    {!! __('Showing') !!}
                    @if ($paginator->firstItem())
                        <span class="font-medium">{{ persian_numbers($paginator->firstItem()) }}</span>
                        {!! __('to') !!}
                        <span class="font-medium">{{ persian_numbers($paginator->lastItem()) }}</span>
                    @else
                        {{ $paginator->count() }}
                    @endif
                    {!! __('of') !!}
                    <span class="font-medium">{{ persian_numbers($paginator->total()) }}</span>
                    {!! __('results') !!}
                </p>
            </div>
