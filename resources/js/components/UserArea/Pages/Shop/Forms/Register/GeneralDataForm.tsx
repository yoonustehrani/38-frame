...
const GeneralDataForm: FC<GeneralDataFormProps> = () => {
    const [inShopField] = useField({name: '__inShop'})
    return (
        <>
            <Input required type="text" name="brand" id="brand" label="نام / برند فروشگاه" className="py-2 px-3"/>
            <Input required type="number" name="founded_in_year" id="year" label="سال شروع فعالیت" className="ltr py-2 px-3" min={1320} max={1403}/>
            ...
            {inShopField.value === 'yes' && <TextArea required name="address" id="address" label="آدرس دقیق فروشگاه" className="py-4 px-3"/>}
        </>
    );
}
