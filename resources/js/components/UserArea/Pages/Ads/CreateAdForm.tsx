    ...
    return (
        <form className="grid md:grid-cols-2 gap-4 md:gap-8 my-6 pb-6 px-3" onSubmit={e => {
            ...
            <FormSection required label={{ for: 'title', text: 'عنوان آگهی' }}>
                <input onChange={e => setAdInfo('title', e.target.value)} maxLength={70} id="title" type="text" name="title" className="form-input py-2 pt-3 px-3" />
            </FormSection>
            ...
            {state.ad.pricing_type === 'price' && (
                <div>
                    <div className="relative">
                        ...
                        <input id="price" type="text" onChange={e => setAdInfo('price', Number(e.target.value))} maxLength={9} minLength={5} name="price" className="form-input ltr py-2 pt-3 px-3" />
                    </div>
            ...
            <FormSection className="col-span-full" label={{ for: 'description', text: 'توضیحات آگهی *' }}>
                <textarea required onChange={e => setAdInfo('description', e.target.value)} maxLength={600} id="description" name="description" className="form-textarea"></textarea>
            </FormSection>
            ...
            <FormSection required label={{ for: 'phone_number', text: 'شماره تلفن' }}>
                <input onChange={e => setAdInfo('phone_number', e.target.value)} id="phone_number" type="text" name="phone_number" className="form-input ltr py-2 pt-3 px-3" placeholder="مثال:‌ 09121234567"/>
            ...
            {[1, 3].includes(state.ad.delivery_type) && (
                <FormSection label={{ for: 'address', text: 'منطقه محل تحویل' }} className="col-span-full">
                    <textarea onChange={e => setAdInfo('address', e.target.value)} id="address" name="address" className="form-textarea"></textarea>
                </FormSection>
            )}
