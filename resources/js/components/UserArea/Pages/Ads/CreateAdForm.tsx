...
import { SiteCategory, UploadingFiles, createAdAction, createAdActionTypes, initialFormValues, sellModeType } from "./types";
...
const CreateAdForm: FC<CreateAdFormProps> = ({handleSubmit, dispatch, handleAddFiles, handleRemoveFile, categories, files}) => {
    ...
    const [sellMode, setSellMode] = useState<sellModeType>('price')

    const dispatchFormEntries = {
        setCategory: (category_id: number) => dispatch({type: createAdActionTypes.SET_CATEGORY, payload: category_id}),
    }
    return (
        <form className="grid md:grid-cols-2 gap-4 md:gap-8 my-6 pb-6 px-3" onSubmit={handleSubmit}>
            <FormSection required label={{ for: 'category', text: 'دسته بندی آگهی' }}>
                <select onChange={e => dispatchFormEntries.setCategory(Number(e.target.value))} defaultValue={initialFormValues.category_id} name="category" id="category" className="w-full bg-transparent border border-black/10 rounded-md py-1 pt-3 px-3">
            ...
            <FormSection required label={{ for: 'title', text: 'عنوان آگهی' }}>
                <input maxLength={70} id="title" type="text" name="title" className="w-full bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3" />
            </FormSection>
            <div className="col-span-full">
                <div className="relative md:w-1/2">
                    ...
                    <div onClick={() => fileInputRef.current?.click()} className="w-full bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3 cursor-pointer">
            ...
            <div className="col-span-full">
                <FormControl>
                    ...
                    <RadioGroup
                        ...
                        value={sellMode}
                        onChange={e => setSellMode(e.target.value as sellModeType)}
                    >
            ...
            {sellMode === 'price' && (
                <div>
                    <div className="relative">
                        ...
                        <input id="price" type="number" onChange={(e) => setPrice(e.target.valueAsNumber)} min={0} maxLength={9} step={10000} name="price" className="w-full bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3" />
                    </div>
            ...
            <FormSection className="col-span-full" label={{ for: 'description', text: 'توضیحات آگهی' }}>
                <textarea maxLength={600} id="description" name="description" className="w-3/4 h-32 bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3"></textarea>
            </FormSection>
            ...
            <FormSection required label={{ for: 'phone_number', text: 'شماره تلفن' }}>
                <input id="phone_number" type="phone_number" name="phone_number" className="w-full [direction:ltr] placeholder:text-right text-left bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3" placeholder="مثال:‌ 09121234567"/>
            ...
            <FormSection label={{ for: 'address_line', text: 'تلفن ثابت' }}>
                <input id="address_line" type="address_line" name="address_line" className="w-full [direction:ltr] placeholder:text-right text-left bg-transparent border border-black/10 rounded-md py-2 pt-3 px-3" placeholder="همراه با کد استان"/>
            </FormSection>
